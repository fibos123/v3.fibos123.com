...
const Header = () => {
  ...
  return (
    <div className="relative">
          ...
          <div className="hidden sm:block pl-4">
            {menu.map(item =>
              <Link href={item.path} key={item.path}>
                <a className={"font-medium text-gray-500 hover:text-indigo-600 transition duration-150 ease-in-out p-4 " + (firstPath === item.path ? 'text-indigo-600' : '')} href={item.path}>
                  <i className={"w-6 pr-1 " + item.icon}></i>
                  {item.name}
                </a>
              </Link>
            )}
      ...
      <div
        className={"absolute top-0 inset-x-0 p-2 transition transform origin-top-right sm:hidden " + (isOpen ? 'block' : 'hidden')}
      >
          ...
          >
            ...
            <div className="px-2 pt-2 pb-3">
              {menu.map(item =>
                <Link href={item.path} key={item.path}>
                  <a
                    className={"block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-indigo-600 hover:bg-gray-50 focus:outline-none focus:text-indigo-600 focus:bg-gray-50 transition duration-150 ease-in-out " + (firstPath === item.path ? 'text-indigo-600' : '')}
                    href={item.path}>
